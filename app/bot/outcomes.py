...
#: Без строки "replaced" reason_label() вернёт сырое английское слово
#: в /stats и /history среди русских подписей.
EXIT_REASONS = {
    "reverse": "обратная дивергенция",
    "same": "новая дивергенция того же направления",
    "replaced": "замена контрольной точки",
    "timeout": "таймаут",
}
...
class OutcomeStore:
    ...
    async def open_for_signal(self, signal: Signal) -> Outcome | None:
        ...
        async with self._lock:
            # INSERT OR IGNORE: degree записывается при ПЕРВОМ появлении сигнала
            # и дальше не обновляется. Это решение, а не недосмотр.
            #
            # Движок переигрывает окно целиком, и по мере ухода истории за левый
            # край окна опорная точка теряется, а цепочка при пересчёте
            # укорачивается. Значит degree одного и того же сигнала со временем
            # может только уменьшаться. Значение, посчитанное при первом
            # появлении, опирается на больший объём истории; обновление на более
            # позднее систематически портило бы статистику дрейфом окна.
            # НЕ «чинить» это заменой на UPSERT.
